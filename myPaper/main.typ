// =============================================================================
// Research Paper: Illuminating the Black Box
...
#align(center)[
  ...
  #text(size: 16pt, weight: "bold")[
    Illuminating the Black Box: A Multi-Method Explainable AI Framework with Uncertainty Quantification for 3D Brain Tumor Segmentation
  ]
  #v(1.5em)
  #text(size: 11pt)[Yogi Amitkumar Patel]
  #v(0.3em)
  #text(size: 9pt)[
    School of Architecture, Computing and Engineering \
    University of East London \
    #link("mailto:u2536809@uel.ac.uk")[u2536809\@uel.ac.uk]
  ]
  #v(2em)
]
...
+ *Multi-perspective explainability:* We implement and quantitatively compare six complementary XAI techniques---Grad-CAM, Guided Backpropagation (GBP), Guided Grad-CAM, Layer-wise Relevance Propagation (LRP), Occlusion Sensitivity, and Monte Carlo (MC) Dropout---providing gradient-based, perturbation-based, and probabilistic perspectives on model decision-making.

+ *Quantitative XAI validation:* We introduce a systematic evaluation framework using four metrics (Pointing Game, Saliency Coverage, Saliency IoU, and Weighted Dice) that objectively assess whether the model "looks at the right place for the right reasons."

+ *Uncertainty-aware explainability:* We integrate MC Dropout uncertainty quantification with structural attribution methods (LRP), enabling identification of _"brittle"_ decision regions where high feature importance intersects with high predictive variance---a critical clinical red flag.

+ *VR-ready volumetric pipeline:* All XAI outputs are exported as spatially-aligned NIfTI volumes, ready for immersive 3D visualization through platforms such as 3D Slicer and SlicerVR.

The remainder of this paper is organized as follows: Section 2 reviews related work in 3D segmentation architectures, XAI methods, and immersive visualization. Section 3 details our proposed framework. Section 4 describes the experimental setup. Section 5 presents quantitative results. Section 6 discusses findings and limitations. Section 7 concludes with future directions.
...
Our framework implements a three-stage pipeline: (1) volumetric tumor segmentation using SegResNet, (2) multi-method XAI analysis, and (3) VR-compatible export. This section details the architecture and each XAI component.

== SegResNet Architecture

The backbone of our framework is SegResNet, a 3D encoder-decoder architecture that processes four-channel MRI volumes ($160^3$ voxels) to produce three-channel segmentation masks corresponding to the Whole Tumor (WT), Tumor Core (TC), and Enhancing Tumor (ET) regions.
...
The preprocessing pipeline applies four operations: (1) foreground cropping to reduce spatial dimensions from ~$240^3$ to ~$160^3$ by removing background; (2) channel-first formatting for PyTorch compatibility $[4, 160, 160, 160]$; (3) per-channel z-score normalization using non-zero voxel statistics; and (4) training-only random augmentations including random flips, rotations, intensity scaling, Gaussian noise, and intensity shifts.

== Training Configuration

Training was conducted on a single NVIDIA RTX 5880 Ada (48 GB VRAM) with Automatic Mixed Precision (AMP). An evolutionary prototyping phase on a 250-patient subset validated the pipeline before full-scale training.
...
@tab:seg_results presents the segmentation performance of SegResNet on the held-out test set (126 patients).
...
#figure(
  table(
    ...
    [WT], [0.435], [0.839], [0.003], [0.00006],
    [TC], [0.496], [0.913], [0.007], [0.00006],
    [ET], [0.447], [0.858], [0.011], [0.00003],
  ),
