...
#figure(
  [
    ...
    #box(width: 60%)[
      ...
      #align(center)[#text(size: 0.85em)[(c) Validation Whole Tumour (WT) Dice]]
    ]
  ],
...
#figure(
  table(
    ...
    [*Sensitivity (Recall)*],
    [$ "TP" / ("TP" + "FN") $],
    [The true positive rate. Measures the fraction of actual tumour voxels correctly identified to prevent missed diagnoses.],
    [*Specificity*],
    [$ "TN" / ("TN" + "FP") $],
...
Beyond structural accuracy, the generated gradient-based visual attributions and the MC Dropout variance maps ($sigma^2$) are quantitatively assessed utilising custom validation metrics:
...
#figure(
  table(
    ...
    [Binary test returning 1 if the highest activation peak in the saliency map $L$ falls inside the tumour region $G$.],
    [*Saliency Coverage*],
    [$ (sum_(x in G) L(x)) / (sum_x L(x)) $],
    [Calculates the percentage of total salient relevance mass concentrated inside the true tumour boundary.],
    [*Saliency IoU*],
