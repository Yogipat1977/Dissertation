...
The interpretation of volumetric medical data has historically been analysed on 2D screens, which causes a lot of cognitive stress. According to Khedir et al. @11083598, traditional slice-by-slice navigation requires clinicians to mentally reconstruct volumetric architecture. This process is time-consuming and prone to spatial errors when assessing the depth of the tumours relative to prominent brain regions. Even though 3D Convolutional Neural Networks (CNNs) can process this volumetric data, the representation of their outputs remains largely stuck in 2D.
...
Additionally, the idea of interactive AI is essential to current medical systems. Static heatmaps (like standard Grad-CAM) offer *"take it or leave it"* explanations. VR environments, on the other hand, make *Human-in-the-Loop (HITL)* interaction easier. Immersive interfaces enable clinicians to interrogate the model, e.g., by digitally "erasing" a portion of the input volume to identify prediction changes @orsmaa2025. This interactivity transforms XAI from a static report into a dynamic conversation between the clinician and the AI, allowing the correction of model errors in real time @orsmaa2025.
